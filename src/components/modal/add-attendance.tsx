...
export default function AddAttendanceModal({
  ...
  const totalPresent = useMemo(() => {
    let count = 0;
    matrix.forEach((r) => SESSIONS.forEach((n) => { if ((r as any)[`s${n}`] === "present") count++; }));
    return count;
  }, [matrix]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (matrixRes.error) { setError(matrixRes.error.message); setMatrix([]); }
        ...
        const rows = (matrixRes.data ?? []).map((r: any) => ({
          ...r,
          s1: (r.s1 ?? "unmarked"),
          s2: (r.s2 ?? "unmarked"),
          s3: (r.s3 ?? "unmarked"),
  ...
  const saveCell = async (studentId: string, sessionNo: number, newStatus: Status, remark?: string) => {
    ...
    setMatrix((rows) =>
      rows.map((r) =>
        r.student_id === studentId ? ({ ...r, [`s${sessionNo}`]: newStatus } as any) : r
      )
    );
    ...
    const key = keyFor(studentId, sessionNo);
    if (error) {
      // revert on error
      setMatrix((rows) =>
        rows.map((r) =>
          r.student_id === studentId ? ({ ...r, [`s${sessionNo}`]: "unmarked" } as any) : r
        )
      );
      ...
      setRemarksByKey((m) => {
        const next = { ...m };
        if (remark) next[key] = remark; else delete next[key];
        return next;
      });
    }
  };
  ...
  const bulkMarkAll = (status: Status) => {
    ...
    const rows = matrix.map((r) => ({ ...r, [`s${n}`]: status } as any));
    setMatrix(rows);
    // fire-and-forget saves
    rows.forEach((r) => {
      void saveCell(r.student_id, n, (r as any)[`s${n}`]);
    });
  };
  ...
  const bulkCopyLastSession = () => {
    ...
    const rows = matrix.map((r) => ({ ...r, [`s${n}`]: (r as any)[`s${copyFrom}`] } as any));
    setMatrix(rows);
    rows.forEach((r) => {
      void saveCell(r.student_id, n, (r as any)[`s${n}`]);
    });
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-default-100">
              ...
              <div className="ml-auto flex items-center gap-3">
                ...
                {/* <Switch
                  ...
                  onValueChange={(v) => setViewMode(v ? "grid" : "quick")}
                >
                  Grid
                </Switch> */}
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 bg-default-100 p-4 rounded-xl shadow-sm">
                ...
                <div>
                  ...
                  >
                    ...
                    {filteredPrograms.map((p) => (<option key={p.id} value={p.id as any}>{p.name}</option>))}
                  </select>
                </div>
                <div>
                  ...
                  {viewMode === "quick" ? (
                    <div className="mt-3 rounded-xl border border-default-100 overflow-hidden">
                      <div className="max-h-[60vh] overflow-auto">
                        {matrix.map((row) => {
                          const s = (row as any)[`s${activeSession}`] as Status;
                          return (
                          ...
                          <tbody>
                            {matrix.map((row) => (
                              <tr key={row.student_id} className="border-t border-default-100">
                                ...
                                {SESSIONS.map((n) => {
                                  const s = (row as any)[`s${n}`] as Status;
