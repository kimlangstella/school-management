...
// import EditAttendanceModal from '../modal/edit-attendance'; // Removed since unused
...
export default function AttendanceTable({ selectedBranchId, selectedProgramId }: Props) {
    ...
    // FIX: Wrapped in useCallback for dependency array
    ...
    // FIX: Wrapped loadStaticLists in useCallback
    const loadStaticLists = useCallback(async () => {
        ...
        try {
            ...
            setError(msg || 'Unexpected error while loading lists.');
        }
    }, []);

    useEffect(() => {
        loadStaticLists();
    }, [loadStaticLists]); // FIX: Added dependency
    ...
    const loadAttendance = async (
        ...
        try {
            ...
            const msg = e instanceof Error ? e.message : String(e);
            setError(msg || 'Failed to load attendance.');
            setRecords([]);
        } finally {
            setLoading(false);
        }
    ...
    }, [records, selectedBranchId, selectedProgramId, selectedClassroomId, studentQuery, getStudentNameById]); // FIX: Added getStudentNameById
