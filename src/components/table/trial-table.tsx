...
export default function StudentTable({ onEdit }: { onEdit?: (trial: any) => void }) {
...
const handleDelete = async (id: string) => {
  ...
  const { error } = await supabase.rpc('delete_trail_by_id', { _id: id });
...
const fetchTrails = async () => {
  try {
    const { data: trailData, error: trailError } = await supabase.rpc("get_all_trails");
    const { data: usersData, error: usersError } = await supabase.rpc("get_all_users");

    if (trailError || usersError) {
      setError(trailError?.message || usersError?.message || "Unknown error");
      return;
    }

    const trialsArray = Array.isArray(trailData) ? trailData : trailData?.results || [];
    const mapped = trialsArray.map((st: Trial) => ({ ...st }));
    setTrials(mapped);
    setUsers(usersData || []);
...
const getBranchNameById = (id: string): string => {
  ...
  const getProgramNameById = (id: string): string => {
    console.log("Looking for program ID:", id);
    const program = programs.find((p: any) => p.id === id);
    console.log("Matched program:", program);
    return program ? program.name : "Unknown Program";
  };

const getUserNameById = (id: string): string => {
  if (!id || !users?.length) {
    console.warn("⚠️ No users found or ID missing:", id);
    return "Unknown User";
  }

  const user = users.find((u) => String(u.id).trim() === String(id).trim());

  if (!user) {
    console.warn("⚠️ User ID not matched:", id);
    console.log("👤 Users list:", users.map((u) => u.id));
  }
...
useEffect(() => {
  if (trials.length > 0 && branches.length > 0) {
    console.log("🟨 Branches:", branches);
    console.log("🟦 Trials:", trials.map(t => ({ id: t.id, branch: t.branch })));

    const matched = trials
      .filter(
        (trial) =>
          typeof trial.branch === "string" &&
          branches.some(
            (branch) =>
              typeof branch.id === "string" &&
              branch.id.toLowerCase() === trial.branch.toLowerCase()
          )
      )
      .map((trial) => {
        const branch = branches.find(
          (b) =>
            typeof b.id === "string" &&
            trial.branch &&
            b.id.toLowerCase() === trial.branch.toLowerCase()
        );
    ...
    console.log("✅ Matched trials with branch names:", matched);
  }
}, [trials, branches]);
    ...
    const sortedItems = useMemo(() => {
        ...
        const sortColumn: keyof Trials = sortDescriptor.column === "program" ? "program_name" : sortDescriptor.column as keyof Trials;
    ...
    const renderCell = useMemoizedCallback((trial: Trial, columnKey: React.Key) => {
...
case "branch_name": {
  return (
    <Chip
      ...
      variant="flat"
    >
      {getBranchNameById(trial.branch)}
    </Chip>
  );
}
...
case "program_name": {
  return (
    <Chip className="rounded-xl bg-default-100 px-[6px] capitalize text-default-800" size="sm" variant="flat">
      {getProgramNameById(trial.program)}
    </Chip>
  );
}

case "assign_by": {
  const name = getUserNameById(trial.assign_by);
  console.log("👀 Rendering assign_by for:", trial.assign_by, "→", name);
  return (
    <Chip className="rounded-xl bg-default-100 px-[6px] capitalize text-default-800" size="sm" variant="flat">
      {name}
    </Chip>
...
case "handle_by":
  return (
    <Chip className="rounded-xl bg-default-100 px-[6px] capitalize text-default-800" size="sm" variant="flat">
      {getUserNameById(trial.handle_by)}
    </Chip>
  );
            ...
            case "number_student":
                return (
                    <div className="flex items-center justify-center gap-2">
                        <Chip color="success" className={"px-5 font-extrabold"} >{cellValue}</Chip>
                    </div>
                );
...
<Icon
        icon="solar:pen-bold"
        onClick={() => onEdit?.(trial)}  // ✅ call the function passed from parent
        className="cursor-pointer text-default-400"
        height={18}
        width={18}
      />
                    ...
                    <div className="flex items-center gap-4">
                            ...
                            <Popover placement="bottom">
                                ...
                                <PopoverContent className="w-80">
                                    <div className="flex w-full flex-col gap-6 px-2 py-4">
                                        ...
                                        <RadioGroup label="StatusStudent" value={statusFilter}
                        ...
                        <div>
                            <Dropdown>
                                ...
                                <DropdownMenu
                                    aria-label="Sort"
                                    items={headerColumns.filter((c) => !["name", "nationality"].includes(c.uid))}
                                >
                                    {(item) => (
                                        <DropdownItem
                                            key={item.uid}
                        ...
                        <div>
                            <Dropdown closeOnSelect={false}>
                                ...
                                <DropdownMenu
                                    ...
                                    aria-label="Columns"
                                    items={trialColumn.filter((c) => !["actions"].includes(c.uid))}
    ...
    const topBar = useMemo(() => {
        return (
            <div className="mb-[18px] flex items-center justify-between">
                ...
                <AddTrail onSuccess={fetchTrails} />
            </div>
        );
    }, [trialLength]);
