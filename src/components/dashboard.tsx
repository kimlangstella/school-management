...
import {Avatar, Button, cn, ScrollShadow, Spacer, Tooltip} from "@heroui/react";
...
export default function DashboardLayout({children}: {children: React.ReactNode}) {
    ...
    <div className="dark text-foreground bg-background flex h-full w-full">
      ...
      >
        <div className={cn("flex items-center gap-3 px-3", {"justify-center gap-0": isCompact})}>
          ...
          <span className={cn("text-small font-bold uppercase opacity-100", {"w-0 opacity-0": isCompact})}>
            AAA School
          </span>
        </div>
  ...
  {/* Left side: toggle + overview title */}
  ...
  <div className="flex items-center gap-3 px-3">
    ...
    <div className={cn("flex max-w-full flex-col", { hidden: isCompact })}>
      <p className="truncate text-small font-medium text-default-600 capitalize">{userInfo?.name}</p>
      <p className="truncate text-small font-medium text-default-600 capitalize">{userInfo?.profile_urlj}</p>
    </div>
  </div>
</header>
        ...
        <main className="w-full h-screen overflow-hidden">
          <div className="flex w-full h-full overflow-auto flex-col">{children}</div>
        </main>
      </div>
    </div>
  );
